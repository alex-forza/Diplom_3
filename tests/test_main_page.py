...
class TestMainPage:
    @allure.title('Логинимся -> создаем заказ -> проверяем его в личном кабинете ->'
    ...
    @allure.title('Логинимся -> добавляем булку в конструктор ->'
    ...
    @allure.title('Логинимся -> создаем заказ -> идем в ленту заказов -> ищем его в работе -> '
    ...
    @allure.title('Перешли в ленту заказов -> сохранили счетчики -> залогинились -> создали заказ')
